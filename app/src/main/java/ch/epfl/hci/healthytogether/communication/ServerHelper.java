...
public class ServerHelper {
	...
	public static final String URL_LOGIN_GetUserName = URL_PREFIX + "checklogin_getUsername.php";
	...
	//private static final String URL_SYNCDATA = URL_PREFIX + "synch4_GetData_withToken.php"; //synch4_GetData.php"; // Add for synchronization run in background (update the DB with Fitbit's data). Yaliang
	private static final String URL_SYNCDATA = URL_PREFIX + "00_test.php"; //synch4_GetData.php"; // Add for synchronization run in background (update the DB with Fitbit's data). Yaliang
	...
	private static final String URL_CHECK_FITBIT = URL_PREFIX
			+ "checkfitbitauth.php"; //checkfitbitauth.php
	...
	public abstract static class CancelPledgesTask extends AsyncTask<Void, Void, Boolean>
	{
		...
		protected Boolean doInBackground(Void... params) {
			Log.d(TAG, "CancelPledgesTask with fitbit servers for user " + uid);
	...
	AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			/*
			 Log.d(TAG, "doInBackground with fitbit servers for user " + email);
			ArrayList<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
	...
	AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			/*
			Log.i(TAG, "synching with fitbit servers for user " + mUserId);
			ArrayList<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
			...
			try {
				...
				HttpResponse response = httpclient.execute(httppost);
				HttpEntity entity = response.getEntity();
				String responseStr = EntityUtils.toString(entity); // This one is OK. Yaliang **************************************
...
// Get User Name, Yaliang. Jan. 27, 2018. ********************************************
	public abstract static class RetrieveUserNameTask extends
			AsyncTask<Void, Void, String> {
		 ...
		 *            retrieved.
		 */
		public RetrieveUserNameTask(int userId) {
			mUserId = userId;
			safeExecution = false;
			//Log.d("userNameID", String.valueOf(mUserId));
		}
		...
		protected String doInBackground(Void... params) {
			...
			try {
				HttpClient httpclient = new DefaultHttpClient();
				HttpPost httppost = new HttpPost(URL_LOGIN_GetUserName);
				httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs,
						"UTF-8"));
				HttpResponse response = httpclient.execute(httppost);
				HttpEntity entity = response.getEntity();
				...
				if (responseStr.startsWith("OK")) {
					responseStr = Utils.dataCheck(responseStr);
					Constants.USER_NAME_ACQUIRED = true;
					safeExecution = true;
					Constants.userName = responseStr.substring(2);
					return responseStr.substring(2);
				} else {
					safeExecution = true;
					return responseStr;
	...
	public abstract static class SyncBackendWithFitbitTask extends
		...
		@Override
		protected Boolean doInBackground(Void... params) {
			Log.d(TAG, "synching BACKEND with fitbit servers for user " + mUserId);
			/*
			ArrayList<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
			nameValuePairs.add(new BasicNameValuePair("uid", "" + mUserId));
			nameValuePairs.add(new BasicNameValuePair("fid", "" + mBuddyId));
			//nameValuePairs.add(new BasicNameValuePair("OAUTH_STATE", "" + mUserId));
			//nameValuePairs.add(new BasicNameValuePair("state", "" + mUserId));
			...
			try {
				...
				HttpClient httpclient = new DefaultHttpClient();
				HttpPost httppost = new HttpPost(URL_SYNCDATA);
				httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs,
						"UTF-8"));

//Todo: This one is Error. Yaliang *************************************
				HttpResponse response = httpclient.execute(httppost);
				Log.d("SYNC_Data_CHECK", "URL");
				//Log.d("SYNC_Data_CHECK", HttpPost.toString(httppost));//todo: how to check the URL is correct?

				HttpEntity entity = response.getEntity();
				String responseStr = EntityUtils.toString(entity);

				Log.d("SYNC_Data_CHECK",responseStr); // This one is Error. Yaliang ***************
				// return true;
...
*/
/*
			RequestQueue queue = Volley.newRequestQueue(ServerHelper.context);
			String url = "https://reach.bitnamiapp.com/php_HT/00_test.php?uid=" + mUserId;
...
*/
	...
	public abstract static class SendMessageTask extends
		...
		protected Boolean doInBackground(Void... params) {
			...
			try {
				...
				//Log.d("URL_MSG", URL_MSG);
				httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs,
						"UTF-8"));
				//Log.i("Sending","From " + mUserId + " to " + mBuddyId);
				HttpResponse response = httpclient.execute(httppost);
				...
				responseStr = Utils.removeNewLines(responseStr);
				 //Log.i("MSG_SEND_RESPONSE","("+responseStr+")");
				Constants.BUDDY_DECLINED = responseStr.equals("RM");
				 Log.d("SendMessageTask", "MESSAGE_RESPONSE: " + responseStr);
				return responseStr.equals("OK");
			} catch (Exception e) {
				Log.e(TAG, "Error in http connection " + e.toString());
				return false;
	...
	public abstract static class RetrieveAllMessagesTask extends
		...
		protected ArrayList<MessageMeta> doInBackground(Void... params) {
			try {
				...
				Log.e("ServerHelper", "InputSource, OK?************");
				Log.e("Input", "Getting messages from " + mUserId + " to " +
						mBuddyId );
	...
	public abstract static class CheckMessagesTask extends
		...
		@Override
		protected String doInBackground(Void... params) {
			Log.e("checkMessages", "Getting messages of ");
			ArrayList<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
			nameValuePairs.add(new BasicNameValuePair("uid", "" + mUserId));
	...
	public abstract static class RetrieveStepCountTask extends
		...
		/**
		 * @param userId
		 *            the id of the user for which the step count should be
		 *            retrieved.
		 */
		...
		@Override
		protected Integer doInBackground(Void... params) {
			//Log.d("test", "Getting steps of " + mUserId);
			int step = 0;
	...
	public abstract static class RegisterTask extends
		...
		public RegisterTask(String email, String password, String name, String institute) {
			mEemail = email;
			mPassword = password;
			mName = name;
		...
		protected String doInBackground(Void... params) {
			...
			nameValuePairs.add(new BasicNameValuePair("institute", mInstitute));

			try {
				HttpClient httpclient = new DefaultHttpClient();
				Log.d(TAG, "ServerHelper A");
	...
	private static InputStream obtainInputStream(String urlAddress,
		...
		// Output the response of inputStream for Debugging
        /*
        InputStream in = response.getEntity().getContent();
		//Log.e("obtain", "!" + response.getEntity().getContent());
		...
		if(in != null)
		{
            ...
            try {
                while ((read = in.read(reqBuffer, 0, reqLen)) >= 0)
...
//@param userId
//the id of the user for which the badges should be retrieved.
