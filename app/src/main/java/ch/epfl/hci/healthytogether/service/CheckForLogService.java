 ...
 * is finished).
 */
public class CheckForLogService extends WakefulIntentService {

	//private static final String WHAT_HAVE_YOU_BEEN_UP_TO = whats_up;
	private static final String TAG = CheckForLogService.class.toString();

	public CheckForLogService() {
		super("CheckForLogService");
	}

	@Override
	protected void doWakefulWork(Intent arg0) {
		Log.i("CheckForLogService", "displaying notification");
		...
		CheckGroupTask checkGroupTask = new CheckGroupTask(email, userId) {
			...
			private void logNotificationRoutine() {
				if (Constants.newLogin) {
					...
					notifyForLog(getResources().getString(R.string.whats_up));
	 ...
	 * 
	 * */
	private void notifyForLog(String notificationMessage) {
		...
		builder = new NotificationCompat.Builder(CheckForLogService.this)
		...
		Intent resultIntent = new Intent(CheckForLogService.this,
				Main2Activity.class);
		TaskStackBuilder stackBuilder = TaskStackBuilder
				.create(CheckForLogService.this);
				...
				if(!this.safeExecution){
					...
					if (!msg.contains("LOG")) {
						...
						builder = new NotificationCompat.Builder(
								...
								.setOnlyAlertOnce(true)
								.setSmallIcon(R.drawable.notification_icon)
								.setContentTitle(getResources().getString(R.string.log_received_title))
								.setContentText(
										getResources().getString(R.string.log_received_content));
					}

					if (isMessage) {
						resultIntent = new Intent(CheckForLogService.this,
