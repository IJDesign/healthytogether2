...
import ch.epfl.hci.healthytogether.communication.ServerHelper.RetrieveUserNameTask;
...
public class Main2Activity extends Activity {
	...
	ArrayList<PieEntry> entries; // For Piechart
	PieChart pieChart;

	ArrayList<IBarDataSet> yAxis;
	ArrayList<BarEntry> yValues;
	public ArrayList<String> xAxis1;
	BarData data;
	float BarChartMaxViewCount = 7f;
	int BarChartDataCount;
...
////Update the ChatView every one minute, Ref: https://stackoverflow.com/questions/5457186/best-way-to-update-textview-every-minute-on-the-minute
	...
	private Runnable checkForProgressTask = new Runnable() {
		...
		public void run() {
			...
			if (!Utils.isConnectionPresent(Main2Activity.this)) {
				...
				//	Date d = new Date();
					...
					if (Constants.validDate(prefs, previousDay)) {
						previousBtn.setImageDrawable(getResources()
								.getDrawable(R.drawable.yc_previousicon));
					}
				...
				// Todo: Debugging the Error. ****************************************** Yaliang
	...
	private int getFriendId() {
		...
		if (fid < 1) {
			...
			String email = AppContext.getInstance().getEmail();
			CheckGroupTask checkGroupTask;
			checkGroupTask = new CheckGroupTask(email,
		...
		for (int i = 0; i < tab_host.getTabWidget().getChildCount(); i++) {
			if (i == 1) {
				...
				tab_host.getTabWidget()
						.getChildAt(i)
						.setBackgroundResource(R.drawable. mytab_roundedcorners2);//mytab_roundedcorners is for Tab(historical)
				cv = tab_host.getTabWidget().getChildAt(i);
				LinearLayout.LayoutParams currentLayout = (LinearLayout.LayoutParams) cv
						.getLayoutParams();
				currentLayout.setMargins(5, 2, 0, 0);

			} else if (i == 0) {
				// Log.d(TAG,"ONUR, TABS " + i +
				// ": appyling  mytab_roundedcorners");
				tab_host.getTabWidget().getChildAt(i)
						.setBackgroundResource(R.drawable.mytab_roundedcorners); //mytab_roundedcorners is for Tab(daily view)
				cv = tab_host.getTabWidget().getChildAt(i);
				LinearLayout.LayoutParams currentLayout = (LinearLayout.LayoutParams) cv
						.getLayoutParams();
...
/*
			...
			LinearLayout.LayoutParams currentLayout = (LinearLayout.LayoutParams) cv
					.getLayoutParams();
			currentLayout.setMargins(5, 2, 5, 0);
			*/
		}

		//Yaliang, Chang the text color, ref: https://stackoverflow.com/questions/22533061/android-tabhost-change-text-color-style
		// Todo: not working yet, search keyword: android TabHost change tab text color selected deselected *******************************************
		tab_host.setOnTabChangedListener(new OnTabChangeListener() {
			...
			public void onTabChanged(String tabId) {
				Log.i("tab", "test");
				for (int i = 0; i < tab_host.getTabWidget().getChildCount(); i++) {
					tab_host.getTabWidget().getChildAt(i).setBackgroundColor(Color.parseColor("#FF0000")); // unselected
					TextView tv = (TextView) tab_host.getTabWidget().getChildAt(i).findViewById(android.R.id.title); //Unselected Tabs
					tv.setTextColor(Color.parseColor("#ffffff"));
				}

				tab_host.getTabWidget().getChildAt(tab_host.getCurrentTab()).setBackgroundColor(Color.parseColor("#0000FF")); // selected
		...
		tab_host.setOnTabChangedListener(new OnTabChangeListener(){
			...
			public void onTabChanged(String tabId) {
			    ...
			    if(tabId.equals("Floors") || tabId.equals(getResources().getString(R.string.floors_tab))) {
					//Yaliang: Show the historial data, ******************************************
			    	//if(time2refresh(lastRefFloorL))
			    	Constants.VIEW_STEPS = false;
                    //updateBarChart(7); // ####Show the Weekly view of the past data. Yaliang
					//setContentView(R.layout.activity_progress_floors);
					TextView text = (TextView) findViewById(R.id.summary_history);
					text.setText(getString(R.string.historyview_ini));
...
//-------- Get the step value and show in the summary text on the top. Yaliang. *****
					...
					// --------- Interaction on the barChart
					// Ref: https://www.studytutorial.in/android-line-chart-or-line-graph-using-mpandroid-library-tutorial
					barChart.setOnChartGestureListener(new OnChartGestureListener() {
						@Override
						public void onChartSingleTapped(MotionEvent me) {
							//Log.d("data Gesture", "START, x: " + me.getX() + ", y: " + me.getY());
							//Toast.makeText(Main2Activity.this, "Single", Toast.LENGTH_SHORT).show();
						}
						...
						public void onChartGestureEnd(MotionEvent me, ChartTouchListener.ChartGesture lastPerformedGesture) {
							...
							if (lastPerformedGesture != ChartTouchListener.ChartGesture.SINGLE_TAP) {
								//highlightValues(Highlight, null); // or highlightTouch(null) for callback to onNothingSelected(...)
						...
						public void onChartDoubleTapped(MotionEvent me) {
							//Toast.makeText(Main2Activity.this, "Double", Toast.LENGTH_SHORT).show();
						}
						@Override
	...
	//	creationComplete = true;
		...
		//Test
		RetrieveStepCountTask rStepCountTask = new RetrieveStepCountTask(AppContext
			...
			protected void onPostExecute(final Integer myStepCount) {
				if (this.safeExecution && Utils.isConnectionPresent(Main2Activity.this)) {
					...
					// buddySteps);
					if (mySteps < 0) {
						//Log.d("test", "my steps is less than 0 ");
					} else {

						Log.d("test", "My steps are " + mySteps);
						userTodayStepsCount = mySteps;
						//Todo: Why couldn't the chart be automatically drawn here? You need to touch the chart to trigger it ***************************
	...
	/*	SimpleDateFormat sdf = new SimpleDateFormat("EEEE");
		...
		if (!Constants.validDate(prefs2, previousDay)) {
			previousBtn.setImageDrawable(getResources().getDrawable(
					R.drawable.yc_previousicon_nonclickable));
		}
	}
...
//Todo: How to call the RetrieveUserNameTask in the ServerHelper.java? ****************************
	public void getUserName()
	{
		RetrieveUserNameTask rUserNameTask = new RetrieveUserNameTask(AppContext.getInstance().getUserId()) {
			@Override
			protected void onPostExecute(String result) {
				if (result != null) {
					//Log.d("Username", result);
					TextView text = (TextView) findViewById(R.id.summary_daily);
					text.setText(Constants.userName + ", " + getString(R.string.dailyview_t) + " " + getString(R.string.dailyview_a));
	...
	protected void showBuddyName(int friendId) 
	{
		if(Constants.BUDDY_NAME_ACQUIRED)
		{
			...
			//Set and show (user name) in the Header (such as: Carl1, you have taken). Yaliang, ****************************************
			//Todo: How to get the user name (instead of the buddyName)?
			/*
			TextView text = (TextView) findViewById(R.id.summary_daily);
			text.setText(Constants.buddyName + getString(R.string.dailyview_a));
			text = (TextView) findViewById(R.id.textViewOtherFloors);
			text.setText(Constants.buddyName);
			*/
		}
		...
		@SuppressLint("StaticFieldLeak") RetrieveExistingBuddyInfoTask task = new RetrieveExistingBuddyInfoTask(AppContext.getInstance().getUserId())
		{
				...
				} else if (initialSyncDialog != null) 
				{
					...
					//	congratulationsDisplayRequest = false;
						...
						else if(congratulationsDisplayRequest[1])
						{
							...
							displayCongratulations(0);// disable by Yaliang, March 21, 2018
						}
	...
	// Todo: RetrieveStepCountTask, How to get the myStepCount??? *********************************
	public void updatePledgeAfterSync(int type)
	{
		switch(type)
		{
		case 0:
			@SuppressLint("StaticFieldLeak") RetrieveStepCountTask task = new RetrieveStepCountTask(AppContext
					.getInstance().getUserId(), 0) {
			...
			break;
		case 1:
			@SuppressLint("StaticFieldLeak") RetrieveFloorCountTask ftask = new RetrieveFloorCountTask(AppContext
					.getInstance().getUserId(), 0) {
	...
	public void updatePledgeBar(final int type) {
		...
		case 0:
			...
			if (pledgeAmount == 0) {
				@SuppressLint("StaticFieldLeak") GetStepPledgeTask task = new GetStepPledgeTask(AppContext
		...
		case 1: //floor
			...
			//look at database. Has the other teammate pledged? if so, update pledgeAmount
			if (pledgeAmountFloor == 0) {
				GetFloorPledgeTask task;
				task = new GetFloorPledgeTask(AppContext
						.getInstance().getUserId()) {
	...
	public void onPledgeConfirmButtonClicked(View v)
	{
		...
		else
		{
			if(pledgeMode == 0)
			{
				...
				@SuppressLint("StaticFieldLeak") SetStepPledgeTask task = new SetStepPledgeTask(
			...
			else
			{
				pledgeAmountFloor = Constants.PLEDGE_AMOUNT_FLOOR[spinnerFloor.getSelectedItemPosition()];
	  			@SuppressLint("StaticFieldLeak") SetFloorPledgeTask task = new SetFloorPledgeTask(
	  					AppContext.getInstance().getFriendId(),
	  					pledgeAmountFloor) {
	  				@Override
	  				protected void onPostExecute(Boolean result) 
...
/*						Toast.makeText(Main2Activity.this,
	...
	private boolean sendNewLog(final String queryEntry,
		...
		@SuppressLint("StaticFieldLeak") SendTimelyLogTask task = new SendTimelyLogTask(uid, fid, sqlText, wholeString) {
	...
	public void deFriend() {
		...
		@SuppressLint("StaticFieldLeak") DeFriendTask task = new DeFriendTask(AppContext.getInstance()
	...
	//Todo: Need to duplicate the updateDatePanel for the History view
// Date Panel, ****************************
	public String updateDatePanel() {
		...
		Date d = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("EEEE");
		SimpleDateFormat datef = new SimpleDateFormat("dd-MMMM"); // Date Formate Ref: https://stackoverflow.com/questions/9277747/android-simpledateformat-how-to-use-it
		...
		// ####Yaliang, Update the date in the "dateText" panel **************
		String datetext = new String();
		String reportingdatetext = new String();
		if (datePivot == 0) {
			//dateTxt.setText(getResources().getString(R.string.today));
			datetext = getResources().getString(R.string.today);
			datetext = datetext + " ";
		} else if (datePivot == 1) {
			//dateTxt.setText(getResources().getString(R.string.yesterday));
			datetext = getResources().getString(R.string.yesterday);
			datetext = datetext + " ";
		} else {
			//dateTxt.setText(sdf.format(yesterday) + ", "
					//+ datef.format(yesterday));
			datetext = sdf.format(yesterday); //+ ", " + datef.format(yesterday);
			datetext = getString(R.string.dailyview_o) + " " + datetext + ", ";
		}

		//Todo: The original data pannel in HT
		//TextView dateTxt = (TextView) findViewById(R.id.dateText);
		//dateTxt.setText(datetext);

		//** Display the Date below the Piechart
		TextView dateTxt2 = (TextView) findViewById(R.id.showthedate);
		dateTxt2.setText(datef.format(yesterday));

		// Show it in the "summary_daily" header (above the Piechart)
		TextView text = (TextView) findViewById(R.id.summary_daily);
		text.setText(Constants.userName + ", " + datetext + getString(R.string.dailyview_a));

		// Check if "the previous day" is valid with respect to game acceptance date
		//Todo: Need to disable the following code???????????? Yaliang **************
		...
		//## Determine how many data should be retrieved. ***************
		if (datePivot == 6 || !Constants.validDate(prefs, previousDay)) {
			previousBtn.setImageDrawable(getResources().getDrawable(
					R.drawable.yc_previousicon_nonclickable));
		} else {
			previousBtn.setImageDrawable(getResources().getDrawable(
					R.drawable.yc_previousicon));
		}

		if (datePivot == 0) {
			nextBtn.setImageDrawable(getResources().getDrawable(
					R.drawable.yc_nexticon_nonclickable));

		} else {
			nextBtn.setImageDrawable(getResources().getDrawable(
					R.drawable.yc_nexticon));
		}
...
/*		SimpleDateFormat sdf = new SimpleDateFormat("EEEE");
		...
		//Todo: Retrieved the steps of the specific date. Yaliang *****************************
		//Log.i("test", String.valueOf(myStepHistory[datePivot]));
		//Log.i("test", String.valueOf(datePivot));
		for (int i = 0; i < myStepHistory.length; i++)
		{
			//Log.i("test2", String.valueOf(myStepHistory[i]));
				//System.out.println(myStepHistory[i]);
		}

		//Update the Chart
		//Todo: Add function for adapting to the Daily or Weekly view, Yaliang ********************
		//updatePieChart(myStepHistory[datePivot]);
		//pd = new ProgressDialog(Main2Activity.this);
		//pd.setMessage("loading");
		Load_data_draw_Piechart(datePivot);

		updateDisplay(datePivot == 0);
		showProgressDialog(false);
	}

	public void onUpdateFitbitDataClicked(View v) {
	...
	public void showLaterHistory(View v) {
		...
		//Update the Chart
		//Todo: Add function for adapting to the Daily or Weekly view *************************
		//updatePieChart(myStepHistory[datePivot]);
		//pd = new ProgressDialog(Main2Activity.this);
		//pd.setMessage("loading");
		Load_data_draw_Piechart(datePivot);
	...
	private void checkFitbitAuthentication() {
		@SuppressLint("StaticFieldLeak") CheckFitbitAuthenticationTask authTask = new CheckFitbitAuthenticationTask(
				AppContext.getInstance().getUserId())
		{

			protected void onPostExecute(Boolean success)
			{
				Log.d("authentication", "ini");
				if(!this.safeExecution)
				{
					if(!Utils.isConnectionPresent(Main2Activity.this))
					{
	...
	private void writeFloorProgress(final int pivotIndex) {
		@SuppressLint("StaticFieldLeak") RetrieveFloorCountTask task = new RetrieveFloorCountTask(AppContext
				.getInstance().getUserId(), pivotIndex) {
	...
	private void updateStepBadges() {
		RetrieveStepCountTask task;
		task = new RetrieveStepCountTask(AppContext
...
//		Log.i(TAG,"drawTopBar Layout: otherSteps = " + otherSteps);
			...
			acceptBtn.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) 
				{
					if(AcceptBuddyActivity.invitationSelected)
					{
						...
						Log.i("Main2Activity","textViewBuddyEmail.getText = " + inviterEmail);//textViewBuddyEmail.getText());
	...
	// -------- Retrieve JSON data and plot the barchart
// Ref: https://freebieslearning.net/android-2/android-bar-radar-chart-volley-php-json/
	...
	public void load_data_draw_barchart() {
				...
				new Response.Listener<String>() {
					...
					public void onResponse(String response) {
						...
						try {
							...
							BarChartDataCount = jsonarray.length();
							//Log.d("string",response);
							for(int i=0; i < jsonarray.length(); i++) {

								float val_user = 0.0f;
								float val_buddy = 0.0f;
								String score = "0";
								String score_buddy = "0";
								int j = i; //jsonarray.length() - i;
								JSONObject jsonobject = jsonarray.getJSONObject(j);

								if(jsonobject.has("user")) {
									score = jsonobject.getString("user").trim();
									val_user = Float.valueOf(score);
								}

								if(jsonobject.has("buddy")){
									score_buddy = jsonobject.getString("buddy").trim();
									val_buddy = Float.valueOf(score_buddy);
								}

								String date = jsonobject.getString("date").trim();

								//Log.d("data", date + "," + score + "," + score_buddy);

								//Create the Array for barchart, by Yaliang. Jan.10.2018.
								xAxis1.add(date);

								//yValues.add(new BarEntry(i,Float.valueOf(score)));
								yValues.add(new BarEntry(i,new float[]{val_user, val_buddy}));
						...
						// Ref: https://github.com/PhilJay/MPAndroidChart/blob/master/MPChartExample/src/com/xxmassdeveloper/mpchartexample/BarChartActivity.java
						//barChart.setOnChartValueSelectedListener(this);
						...
						//Ref. https://github.com/PhilJay/MPAndroidChart/wiki/Modifying-the-Viewport
						barChart.setFitBars(true);
						barChart.setAutoScaleMinMaxEnabled(false);
						barChart.setDrawGridBackground(false);

						barChart.setMaxVisibleValueCount(7); // Todo: What does this for??? *********
						barChart.setDrawBarShadow(false);
						...
						barChart.setVisibleXRange(BarChartMaxViewCount,5f);
						barChart.setVisibleXRangeMaximum(BarChartMaxViewCount);// Set the Viewport, Yaliang ******************
						barChart.moveViewToX(BarChartDataCount);



						pd.hide(); // hid the progress popup info
					}
				},
				new Response.ErrorListener() {
					...
					public void onErrorResponse(VolleyError error) {
						if(error != null){
							...
							Toast.makeText(getApplicationContext(), "Sorry, something went wrong. Please try again latter.", Toast.LENGTH_LONG).show();
							pd.hide();
						}
					}
				}
		...
		//This is a trigger for this function.
		...
		StringRequest stringRequest = new StringRequest(Request.Method.POST,
				...
				new Response.Listener<String>() {
					...
					public void onResponse(String response) {
						...
						if ( DailyStepGoal <= Steps ) {
							DailyStepGoal = Steps;
						}

						entries.add(new PieEntry((int) Steps, 0));
						entries.add(new PieEntry((int) (DailyStepGoal - Steps), 1));
						pieChart.notifyDataSetChanged(); // Update the Chart

						PieDataSet dataset = new PieDataSet(entries, "Steps of the day");

						//## Set the color of the slices
						final int[] MY_COLORS = {Color.rgb(0,118,191),Color.rgb(192,192,192)};
						...
						//## Draw the center text and hole *******
						pieChart.setCenterText(generateCenterSpannableText(Steps));
						...
						pieChart.setTouchEnabled(false); // Disable the interactions. Yaliang ***************
					...
					private SpannableString generateCenterSpannableText(int Steps) {
						//String text = new String();
						//text = Integer.toString(Steps) + "steps";
						//SpannableString s = new SpannableString(Integer.toString(Steps) + getString(R.string.dailyview_b));
						//Todo: How to hide the other elements in the Chart?

						String centerText = "";
						float textSize;
						// Break the center text into two lines
						if (Steps > 0){
							centerText = Integer.toString(Steps) + "\n" + getString(R.string.dailyview_b);
							textSize = 3.2f;
						} else {
							centerText = getString(R.string.dailyview_c) + "\n" + getString(R.string.dailyview_d);
							textSize = 1.8f;
						}
						//centerText = centerText + "\n" + getString(R.string.dailyview_b);
						int index = centerText.indexOf("\n"); //for break the text into two lines

//Set the styles of the texts
						SpannableString tempSpannable = new SpannableString(centerText);
						tempSpannable.setSpan(new RelativeSizeSpan(textSize), 0, index, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
						tempSpannable.setSpan(new RelativeSizeSpan(1.8f), index, tempSpannable.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
						tempSpannable.setSpan(new ForegroundColorSpan(Color.DKGRAY),
